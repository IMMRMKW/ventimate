...
namespace airpurifier {
    ...
    {
        Serial.println("Air Purifier update");
        uint32_t cur_time = millis();
        uint8_t power_setpoint = 0;

        if (!manual_) {
            // Simple controller that determines the required fan power if the air purifier is on automatic mode.
    ...
    uint8_t AIRPURIFIER::calculate_air_quality_index()
    {
        uint8_t index = 0;
        if (sensor_co2_) {
